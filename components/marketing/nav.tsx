...
export function MarketingNav() {
  ...
  useEffect(() => {
    const c = getCookie("riden_lang")
    if (c && SUPPORTED_LANGS.includes(c as Lang)) setLang(c as Lang)
  }, [])
  ...
  function switchLang(next: Lang) {
    ...
    // Optionally refresh so server components re-render with new lang
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-10 h-16 flex items-center justify-between">
        ...
        <ul className="hidden md:flex items-center gap-7 list-none">
          {[
            ...
            <li key={href}>
              <Link href={href} className="font-mono text-[12px] uppercase tracking-[0.08em] text-white/70 hover:text-white transition-colors no-underline">
                {label}
              </Link>
            </li>
          ))}
                ...
                {SUPPORTED_LANGS.map(l => (
                  <button
                    ...
                    className={`block w-full text-left px-3 py-1.5 text-[12px] whitespace-nowrap ${
                      l === lang ? "bg-primary/15 text-primary" : "text-white/80 hover:bg-white/5 hover:text-white"
                    }`}
                  >
                    {LANG_NAMES[l]}
                  </button>
        ...
        <div className="md:hidden fixed inset-0 z-50 bg-[#0A0C10] flex flex-col">
          ...
          <div className="flex-1 p-6 flex flex-col gap-3">
            {[
              ...
              <Link key={href} href={href} onClick={() => setMobileOpen(false)} className="py-2 text-white font-display text-[20px] no-underline">
                {label}
              </Link>
            ))}
            <div className="mt-6 flex flex-wrap gap-2">
              {SUPPORTED_LANGS.map(l => (
                <button key={l} onClick={() => switchLang(l)}
                  className={`h-8 px-2.5 rounded-md text-[12px] font-mono border ${
                    l === lang ? "bg-primary/15 border-primary/40 text-primary" : "border-white/10 text-white/60"
                  }`}>
                  {LANG_NAMES[l]}
                </button>
              ))}
            ...
            <div className="mt-6 space-y-3">
              <a href="https://dmc.riden.me/dmc/login" className="block text-center py-3 rounded-md border border-white/10 text-white no-underline">Log in</a>
              <a href="https://dmc.riden.me/dmc/register" className="block text-center py-3 rounded-md bg-primary text-white no-underline">Begin trial ↗</a>
            </div>
          </div>
        </div>
      )}

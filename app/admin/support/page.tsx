...
export default function AdminSupportPage() {
  ...
  const filtered = mockTickets.filter(t =>
    ...
    t.dmc.toLowerCase().includes(searchQuery.toLowerCase())
  )
  ...
  return (
    ...
    >
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-[22px] font-semibold text-foreground">Support Tickets</h1>
          <p className="text-sm text-muted mt-0.5">{filtered.length} tickets</p>
        </div>
        ...
        <table className="w-full">
          <thead>
            <tr className="bg-background">
              {["Ticket", "DMC", "Priority", "Status", "Created"].map(h => (
            ...
            {filtered.map((t, i) => (
              ...
              >
                <td className="py-3 px-4">
                  ...
                  <p className="text-[12px] text-muted font-mono">{t.id}</p>
                </td>
                <td className="py-3 px-4 text-sm text-muted">{t.dmc}</td>
                <td className="py-3 px-4">
                  <span className={`inline-block px-2 py-0.5 rounded-full font-mono text-[10px] uppercase ${priorityColors[t.priority as keyof typeof priorityColors]}`}>{t.priority}</span>
                </td>
          ...
          <>
            ...
            <motion.div initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }} transition={{ type: "spring", damping: 25, stiffness: 200 }} className="fixed right-0 top-0 h-full w-full max-w-[440px] bg-surface border-l border-border z-50 flex flex-col">
              <div className="h-14 border-b border-border px-5 flex items-center justify-between">
                <h2 className="text-[15px] font-semibold text-foreground">Ticket Details</h2>
                <button onClick={() => setSelectedTicket(null)} className="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-surface-elevated">
              ...
              <div className="flex-1 overflow-y-auto p-5">
                <div className="flex items-start gap-4 mb-6">
                  ...
                  <div>
                    <h3 className="text-[17px] font-semibold text-foreground">{selectedTicket.subject}</h3>
                    <p className="text-[13px] text-muted mt-0.5">{selectedTicket.dmc}</p>
                    <div className="flex items-center gap-2 mt-2">
                      <StatusBadge status={selectedTicket.status} />
                      <span className={`inline-block px-2 py-0.5 rounded-full font-mono text-[10px] uppercase ${priorityColors[selectedTicket.priority as keyof typeof priorityColors]}`}>{selectedTicket.priority}</span>
                    </div>
                  </div>
                </div>
                <div className="space-y-3">
                  <div className="flex items-start gap-3 py-2.5 border-b border-border">
                    ...
                    <div>
                      <p className="font-mono text-[10px] uppercase text-muted tracking-wider">Created</p>
                      <p className="text-[13px] text-foreground mt-0.5">{fmtDate(selectedTicket.created_at)}</p>
                    </div>
                  </div>
                </div>
              </div>
              <div className="border-t border-border p-4 flex gap-3">
                <Button variant="secondary" className="flex-1" onClick={() => setSelectedTicket(null)}>Close</Button>
                <Button className="flex-1 bg-primary">Reply</Button>
              </div>
            </motion.div>
          </>
        )}

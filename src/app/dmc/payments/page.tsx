  ...
  return (
    <motion.div initial={{opacity:0,y:8}} animate={{opacity:1,y:0}} transition={{duration:0.2}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:16,marginBottom:24}}>
        {[
          {label:"Confirmed",count:payments.filter(p=>p.status==="confirmed").length,color:"#1D9E75"},
          {label:"Proof Uploaded",count:payments.filter(p=>p.status==="proof_uploaded").length,color:"#3b82f6"},
          ...
          <div key={s.label} style={{background:"var(--bg-surface,#111)",border:"1px solid var(--border,#242)",borderLeft:`3px solid ${s.color}`,borderRadius:12,padding:16}}>
            <div style={{fontFamily:"monospace",fontSize:10,color:"var(--text-2,#999)",textTransform:"uppercase",letterSpacing:1,marginBottom:6}}>{s.label}</div>
            <div style={{fontFamily:"monospace",fontSize:28,fontWeight:700,color:"var(--text-1,#f0f0f0)"}}>{s.count}</div>
          </div>
        ))}
      </div>
          ...
          <tbody>{filtered.map((p,i)=>(
            <tr key={i} style={{borderBottom:"1px solid rgba(255,255,255,0.04)",transition:"background 120ms"}}
              ...
              <td style={{padding:"12px 16px"}}>
                <span style={{display:"inline-flex",alignItems:"center",gap:5,padding:"3px 10px",borderRadius:99,fontSize:11,fontFamily:"monospace",background:BG[p.status],color:COL[p.status]}}>

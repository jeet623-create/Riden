...
const tickets = [
  { id:"TK-001", subject:"Driver arrived late for airport transfer", status:"replied" as const, date:"2026-04-16", messages:[
    ...
    { from:"support", text:"We sincerely apologize for the inconvenience. We have reviewed the trip log and issued a warning to the driver. A credit has been added to your account.", time:"10:30" },
  ]},
  { id:"TK-002", subject:"Need to change vehicle type for booking BK-2026-003", status:"open" as const, date:"2026-04-15", messages:[
  ...
  { id:"TK-003", subject:"Payment proof not received from operator", status:"open" as const, date:"2026-04-13", messages:[
    { from:"dmc", text:"It's been 48 hours since trip T102-2 closed but we still haven't received payment confirmation from Island Express.", time:"16:00" },
  ]},
  { id:"TK-004", subject:"Invoice request for March bookings", status:"closed" as const, date:"2026-04-01", messages:[
    { from:"dmc", text:"Please send consolidated invoice for all March 2026 bookings.", time:"09:00" },
    { from:"support", text:"Invoice has been sent to your registered email. You can also download it from your dashboard.", time:"09:45" },
  ]},
]

const STATUS_STYLE = {
  open:{ bg:"rgba(245,158,11,0.08)", color:"#f59e0b" },
  replied:{ bg:"rgba(59,130,246,0.1)", color:"#3b82f6" },
  closed:{ bg:"rgba(255,255,255,0.05)", color:"#555" },
}
    ...
    <motion.div initial={{ opacity:0, y:8 }} animate={{ opacity:1, y:0 }} transition={{ duration:0.2 }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"320px 1fr", gap:16, height:"calc(100vh - 220px)", minHeight:480 }}>
        ...
        <div style={{ background:"var(--bg-surface,#111)", border:"1px solid var(--border,#242)", borderRadius:12, overflow:"hidden", display:"flex", flexDirection:"column" }}>
          <div style={{ padding:"12px 16px", borderBottom:"1px solid var(--border,#242)", fontFamily:"var(--font-mono,monospace)", fontSize:10, color:"var(--text-2,#999)", textTransform:"uppercase", letterSpacing:1 }}>
            TICKETS
          </div>
          <div style={{ overflowY:"auto", flex:1 }}>
            {tickets.map(tk=>(
              <div key={tk.id} onClick={()=>setSel(tk)}
                style={{ padding:"14px 16px", borderBottom:"1px solid rgba(255,255,255,0.04)", cursor:"pointer", background:sel.id===tk.id?"var(--teal-10,rgba(29,158,117,0.08))":"transparent", borderLeft:sel.id===tk.id?"2px solid #1D9E75":"2px solid transparent", transition:"all 150ms" }}>
                <div style={{ display:"flex", justifyContent:"space-between", marginBottom:4 }}>
                  <span style={{ fontFamily:"var(--font-mono,monospace)", fontSize:11, color:"#1D9E75" }}>{tk.id}</span>
                  <span style={{ display:"inline-flex", padding:"1px 7px", borderRadius:99, fontSize:10, fontFamily:"var(--font-mono,monospace)", background:STATUS_STYLE[tk.status].bg, color:STATUS_STYLE[tk.status].color }}>{tk.status}</span>
                </div>
        ...
        <div style={{ background:"var(--bg-surface,#111)", border:"1px solid var(--border,#242)", borderRadius:12, display:"flex", flexDirection:"column", overflow:"hidden" }}>
          <div style={{ padding:"14px 20px", borderBottom:"1px solid var(--border,#242)", display:"flex", justifyContent:"space-between", alignItems:"center" }}>
            ...
            <span style={{ display:"inline-flex", padding:"3px 10px", borderRadius:99, fontSize:11, fontFamily:"var(--font-mono,monospace)", background:STATUS_STYLE[sel.status].bg, color:STATUS_STYLE[sel.status].color }}>{sel.status}</span>
          </div>
            ...
            {sel.messages.map((m,i)=>(
              <div key={i} style={{ display:"flex", justifyContent:m.from==="dmc"?"flex-end":"flex-start" }}>
                <div style={{ maxWidth:"70%", padding:"10px 14px", borderRadius:12, background:m.from==="dmc"?"rgba(29,158,117,0.12)":"var(--bg-elevated,#1a1a1a)", border:`1px solid ${m.from==="dmc"?"rgba(29,158,117,0.2)":"var(--border,#242)"}` }}>
                  <div style={{ fontSize:11, color:m.from==="dmc"?"#1D9E75":"var(--text-2,#999)", marginBottom:4, fontFamily:"var(--font-mono,monospace)" }}>{m.from==="dmc"?"You":"RIDEN Support"} · {m.time}</div>

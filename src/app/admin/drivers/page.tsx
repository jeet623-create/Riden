    ...
    <AdminShell>
      {lightbox && (
        ...
        >
          ...
          <div onClick={() => setLightbox(null)} style={{ position: 'absolute', top: 24, right: 32, color: 'var(--text-1)', fontSize: 28, cursor: 'pointer' }}>x</div>
        </div>
      )}

      {selected && (
        <div
          style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'flex-end', backdropFilter: 'blur(4px)' }}
          onClick={() => setSelected(null)}
        >
          <div
            onClick={e => e.stopPropagation()}
            style={{ width: 440, height: '100vh', background: 'var(--bg-surface)', borderLeft: '1px solid var(--border)', padding: 28, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 20 }}
          >
            ...
            <div>
              ...
              {selected.vehicle_photo_url
                ...
                : <div style={{ height: 160, background: 'var(--bg-elevated)', borderRadius: 12, display: 'flex', alignItems: 'center', justifyContent: 'center', border: '1px solid var(--border)' }}><span style={{ fontSize: 12, color: 'var(--text-3)' }}>No photo</span></div>
              }
            </div>
            <div style={{ background: 'var(--bg-elevated)', borderRadius: 'var(--r)', padding: 16, display: 'flex', flexDirection: 'column', gap: 10 }}>
              {[['Phone', selected.phone || 'No data'], ['Vehicle', (VL[selected.vehicle_type] || selected.vehicle_type) + ' - ' + selected.vehicle_plate], ['License', selected.license_number || 'No data'], ['Expiry', selected.license_expiry || 'No data'], ['Base', selected.base_location || 'No data']].map(([k, v]) => (
            ...
            {!selected.is_verified && !selected.is_active && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                <Btn variant="teal" onClick={() => approve(selected.id)} disabled={acting}>
                  {acting ? 'Processing...' : 'Approve Driver'}
                </Btn>
                <input
                  value={rejReason}
                  onChange={e => setRejReason(e.target.value)}
                  placeholder="Rejection reason..."
                  style={{ background: 'var(--bg-elevated)', border: '1px solid var(--border)', borderRadius: 'var(--r)', padding: '8px 12px', fontSize: 12, color: 'var(--text-1)', fontFamily: 'var(--font-body)', width: '100%', outline: 'none' }}
                />
                <Btn variant="danger" onClick={() => reject(selected.id)} disabled={acting}>
      ...
      <PageHeader
        ...
        sub="Review and approve drivers"
      ...
      <div style={{ display: 'flex', gap: 6, marginBottom: 20 }}>
        {['all', 'pending', 'active', 'inactive'].map(f => (
          <button
            key={f}
            onClick={() => setFilter(f)}
            style={{ padding: '6px 14px', borderRadius: 20, border: '1px solid ' + (filter === f ? 'var(--teal-20)' : 'var(--border)'), background: filter === f ? 'var(--teal-10)' : 'transparent', color: filter === f ? 'var(--teal)' : 'var(--text-3)', fontSize: 12, cursor: 'pointer', fontFamily: 'var(--font-body)', textTransform: 'capitalize' as const }}
          >
      ...
      {filtered.length === 0 ? (
        <div style={{ background: 'var(--bg-surface)', border: '1px solid var(--border)', borderRadius: 12, padding: 60, textAlign: 'center' }}>
          <Empty icon="driver" message="No drivers in this category" />
        </div>
      ) : (
          ...
          {filtered.map(d => {
            ...
            return (
              <div
                ...
                style={{ background: 'var(--bg-surface)', border: '1px solid ' + (isPending ? 'rgba(245,158,11,0.25)' : 'var(--border)'), borderRadius: 12, overflow: 'hidden', cursor: 'pointer', transition: 'border-color 0.15s' }}
                onMouseEnter={e => { e.currentTarget.style.borderColor = 'var(--border-strong)' }}
                onMouseLeave={e => { e.currentTarget.style.borderColor = isPending ? 'rgba(245,158,11,0.25)' : 'var(--border)' }}
              >
                <div style={{ height: 150, background: 'var(--bg-elevated)', position: 'relative', overflow: 'hidden' }}>
                  ...
                  <div style={{ position: 'absolute', top: 8, right: 8 }}>
                    <Badge status={isActive ? 'active' : isPending ? 'pending' : 'inactive'} />
                  </div>
                </div>
                <div style={{ padding: '12px 14px' }}>
                  ...
                  <div style={{ fontSize: 11, color: 'var(--text-3)' }}>{d.phone || 'No phone'} | {d.base_location || 'No location'}</div>
                </div>
                {isPending && (

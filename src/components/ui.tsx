  ...
  return (
    ...
    >
      ...
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:14 }}>
        <span style={{ fontSize:11, fontFamily:'var(--font-mono)', color:'var(--text-3)', letterSpacing:1, textTransform:'uppercase' as const }}>{label}</span>
      ...
      <div style={{ display:'flex', alignItems:'center', gap:8 }}>
        {trend && <span style={{ fontSize:11, fontFamily:'var(--font-mono)', color:trend.dir==='up'?'var(--green)':trend.dir==='down'?'var(--red)':'var(--text-3)', background:trend.dir==='up'?'var(--green-bg)':trend.dir==='down'?'var(--red-bg)':'transparent', padding:'2px 6px', borderRadius:4 }}>{trend.dir==='up'?'up':trend.dir==='down'?'down':'-'} {trend.pct}</span>}
...
export function Badge({ status, label }: { status:string; label?:string }) {
  ...
  return (
    <span style={{ display:'inline-flex', alignItems:'center', gap:5, padding:'3px 9px', borderRadius:20, fontSize:11, fontWeight:500, background:s.bg, color:s.color, letterSpacing:0.3, whiteSpace:'nowrap' as const, fontFamily:'var(--font-mono)' }}>
...
export function Btn({ children, onClick, variant='primary', size='md', disabled, style:extStyle, icon }: {
  ...
  const V = {
    primary:   { bg:'var(--text-1)',    color:'#000', border:'transparent', hbg:'#ddd' },
    teal:      { bg:'var(--teal)',      color:'#fff', border:'transparent', hbg:'#17a06a' },
    secondary: { bg:'var(--bg-elevated)', color:'var(--text-1)', border:'var(--border-strong)', hbg:'var(--bg-hover)' },
    ghost:     { bg:'transparent',     color:'var(--text-2)', border:'transparent', hbg:'var(--bg-hover)' },
    danger:    { bg:'var(--red-bg)',   color:'var(--red)', border:'rgba(239,68,68,0.25)', hbg:'rgba(239,68,68,0.2)' },
  }
...
export function Panel({ children, style }: { children:React.ReactNode; style?:React.CSSProperties }) {
  return (
    <div style={{ background:'var(--bg-surface)', border:'1px solid var(--border)', borderRadius:'var(--r-lg)', overflow:'hidden', ...style }}>
      {children}
    </div>
  )
}

export function PanelHeader({ title, sub, actions }: { title:string; sub?:string; actions?:React.ReactNode }) {
  return (
    ...
    <div style={{ overflowX:'auto' }}>
      <table style={{ width:'100%', borderCollapse:'collapse' }}>
        <thead>
          <tr>
            {columns.map(col=>(
              <th key={col} style={{ padding:'10px 16px', textAlign:'left' as const, fontSize:10, fontFamily:'var(--font-mono)', color:'var(--text-3)', letterSpacing:1, textTransform:'uppercase' as const, borderBottom:'1px solid var(--border)', whiteSpace:'nowrap' as const, fontWeight:500 }}>{col}</th>
            ))}
          </tr>
        </thead>
...
export function TD({ children, mono, muted, style }: { children:React.ReactNode; mono?:boolean; muted?:boolean; style?:React.CSSProperties }) {
  return (
    <td style={{ padding:'13px 16px', fontSize:13, color:muted?'var(--text-3)':'var(--text-1)', fontFamily:mono?'var(--font-mono)':'var(--font-body)', borderBottom:'1px solid var(--border)', verticalAlign:'middle' as const, ...style }}>
      {children}
    </td>
  )
}
...
export function Input({ value, onChange, placeholder, type='text', style }: {
  ...
  return (
    <input type={type} value={value} onChange={e=>onChange(e.target.value)} placeholder={placeholder}
      style={{ background:'var(--bg-elevated)', border:'1px solid var(--border)', borderRadius:'var(--r)', padding:'8px 12px', fontSize:13, color:'var(--text-1)', outline:'none', fontFamily:'var(--font-body)', transition:'border-color 0.15s', width:'100%', ...style }}

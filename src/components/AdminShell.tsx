...
export function AdminShell({ children }: { children: React.ReactNode }) {
  ...
  async function fetchPending() {
    ...
    try {
      const r = await fetch(`${SUPA}/rest/v1/drivers?is_verified=eq.false&is_active=eq.false&select=id`, {
  ...
  const isActive = (item: typeof NAV[0]) =>
    item.exact ? pathname === item.h : pathname.startsWith(item.h)
    ...
    <div style={{ display: 'flex', minHeight: '100vh' }}>
      <aside style={{
        ...
        <div style={{
          ...
          {!collapsed && (
            <div>
              <div style={{ fontFamily: 'var(--font-brand)', fontSize: 16, letterSpacing: 4, color: 'var(--text-1)', lineHeight: 1 }}>RIDEN</div>
              <div style={{ fontSize: 9, color: 'var(--teal)', letterSpacing: 2, marginTop: 3, fontFamily: 'var(--font-mono)' }}>ADMIN PANEL</div>
            </div>
          )}
          {collapsed && <div style={{ fontFamily: 'var(--font-brand)', fontSize: 10, color: 'var(--teal)', letterSpacing: 2 }}>R</div>}
          <button onClick={() => setCollapsed(!collapsed)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-3)', fontSize: 12, padding: 4, borderRadius: 4, transition: 'color 0.15s' }}
                  ...
                  {!collapsed && (
                    <>
                      ...
                      {item.badge && pendingCount > 0 && (
                        <span style={{ background: 'var(--amber)', color: '#000', fontSize: 10, fontWeight: 700, padding: '2px 6px', borderRadius: 10, fontFamily: 'var(--font-mono)', lineHeight: 1.4 }}>{pendingCount}</span>
                      )}
                    </>
                  )}
                </div>
        ...
        <div style={{ padding: '12px 8px', borderTop: '1px solid var(--border)' }}>
          {!collapsed && (
            <div style={{ padding: '8px 12px', marginBottom: 8 }}>
              <div style={{ fontSize: 11, fontFamily: 'var(--font-mono)', color: 'var(--teal)', letterSpacing: 1 }}>{time}</div>
              <div style={{ fontSize: 10, color: 'var(--text-3)', marginTop: 2 }}>Bangkok TZ</div>
            </div>
          )}
          <button onClick={handleLogout} style={{
            width: '100%', padding: collapsed ? '10px 0' : '9px 12px',
            background: 'transparent', border: '1px solid transparent',
            borderRadius: 'var(--r)', color: 'var(--text-3)', cursor: 'pointer',
            fontSize: 12, display: 'flex', alignItems: 'center', gap: 8,
            justifyContent: collapsed ? 'center' : 'flex-start',
            transition: 'all 0.12s', fontFamily: 'var(--font-body)',
          }}
            onMouseEnter={e => { e.currentTarget.style.background = 'var(--red-bg)'; e.currentTarget.style.color = 'var(--red)'; e.currentTarget.style.borderColor = 'rgba(239,68,68,0.2)' }}

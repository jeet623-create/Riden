...
export function StatusBadge({ status, variant = 'default', showPulse }: StatusBadgeProps) {
  const normalizedStatus = status.toLowerCase().replace(/ /g, '_')
  const c = cfg[normalizedStatus] ?? { bg: 'rgba(255,255,255,0.05)', text: 'var(--text-2)', label: status }
  
  const isInProgress = normalizedStatus === 'in_progress'
  const shouldShowPulse = showPulse ?? isInProgress
  ...
  return (
    <span 
      ...
      style={{
        ...
        fontSize: variant === 'small' ? 10 : 11,
        padding: variant === 'small' ? '2px 8px' : '3px 10px',
        background: c.bg,
        color: c.text,
      }}
    >
